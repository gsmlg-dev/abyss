defmodule Abyss.ServerTest do
  ...
  defmodule Goodbye do
    ...
    def handle_shutdown(socket, state) do
      Abyss.Socket.send(socket, "GOODBYE")
      {:close, state}
    end
  end
  ...
  defmodule ReadOpt do
    ...
    def handle_data(data, socket, state) do
      ...
      Abyss.Socket.send(socket, inspect(Abyss.Socket.getopts(socket, opts)))
      {:close, state}
    end
  end
  ...
  defmodule Whoami do
    ...
    @impl Abyss.Handler
    def handle_connection(socket, state) do
      Abyss.Socket.send(socket, :erlang.pid_to_list(self()))
      {:continue, state}
    end
  end
  ...
  describe "num_connections handling" do
    ...
    test "should emit telemetry events as expected" do
      ...
      assert_receive {:telemetry, [:abyss, :acceptor, :spawn_error], measurements, metadata},
                     1000
  ...
  test "should enumerate active connection processes" do
    ...
    {:ok, pids} = Abyss.connection_pids(server_pid)
    assert Enum.sort(pids) == Enum.sort([pid_1, pid_2])

    :gen_tcp.close(client)
    Process.sleep(100)

    assert {:ok, [pid_2]} == Abyss.connection_pids(server_pid)

    :gen_tcp.close(other_client)
    Process.sleep(100)

    assert {:ok, []} == Abyss.connection_pids(server_pid)
  end
  ...
  describe "shutdown" do
    test "it should stop accepting connections but allow existing ones to complete" do
      ...
      # Make sure the socket has transitioned ownership to the connection process
      Process.sleep(100)
      task = Task.async(fn -> Abyss.stop(server_pid) end)
    ...
    test "it should emit telemetry events as expected" do
      ...
      assert_receive {:telemetry, [:abyss, :listener, :start], measurements, metadata},
                     500
      ...
      assert metadata
             ~> %{
               ...
               local_port: integer(),
               transport_module: Abyss.Transports.TCP,
               transport_options: []
             }

      assert_receive {:telemetry, [:abyss, :acceptor, :start], measurements, metadata},
                     500
      ...
      assert_receive {:telemetry, [:abyss, :acceptor, :stop], measurements, metadata},
                     500
      ...
      assert_receive {:telemetry, [:abyss, :listener, :stop], measurements, metadata},
                     500
    ...
    test "ssl should allow default options to be overridden" do
      {:ok, _, port} =
        start_handler(ReadOpt,
          transport_module: Abyss.Transports.SSL,
          transport_options: [
            send_timeout: 1230,
  ...
  describe "invalid configuration" do
    ...
    test "handshake should fail if the client offers only unsupported ciphers" do
      ...
      {:ok, server_pid} = start_supervised({Abyss, server_args})
      {:ok, {_ip, port}} = Abyss.listener_info(server_pid)
  ...
  defp start_handler(handler, opts \\ []) do
    ...
    {:ok, server_pid} = start_supervised({Abyss, resolved_args})
    {:ok, {_ip, port}} = Abyss.listener_info(server_pid)
    {:ok, server_pid, port}
  end
end
