defmodule Abyss.Server do
  ...
  def suspend(pid) do
    with pool_sup_pid when is_pid(pool_sup_pid) <- acceptor_pool_supervisor_pid(pid),
         :ok <- Abyss.AcceptorPoolSupervisor.suspend(pool_sup_pid),
  ...
  def resume(pid) do
    with :ok <- wrap_restart_child(pid, :listener),
         ...
         :ok <- Abyss.AcceptorPoolSupervisor.resume(pool_sup_pid) do
      :ok
    else
      _ -> :error
    end
  ...
  def init(config) do
    children = [
      {Abyss.ListenerPool, {self(), config}}
      |> Supervisor.child_spec(id: :listener_pool),
      {Abyss.SenderPool, {self(), config}}
