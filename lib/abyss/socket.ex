defmodule Abyss.Socket do
  ...
  called by implementations which are based on `Abyss.Handler`
  """
  @spec handshake(t()) :: Abyss.Transport.on_handshake()
  def handshake(%__MODULE__{} = socket) do
    case socket.transport_module.handshake(socket.socket) do
      ...
      {:error, reason} = err ->
        Abyss.Telemetry.stop_span(socket.span, %{}, %{error: reason})
        err
    end
  end
  ...
  called by implementations which are based on `Abyss.Handler`
  """
  @spec upgrade(t(), module(), term()) :: Abyss.Transport.on_upgrade()
  ...
  """
  @spec peercert(t()) :: Abyss.Transport.on_peercert()
